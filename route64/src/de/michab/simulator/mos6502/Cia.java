/* $Id: Cia.java,v 1.21 2010/02/21 16:59:20 michab66 Exp $
 *
 * Project: Route64
 *
    ...
    DefaultChip
{
   ...
   */
  private int _ciaNumber = ++_ciaCount;
   ...
   *
   */
  private Forwarder _ioPortA = null;


  /**
   *
   */
  private Forwarder _ioPortB = null;
  ...
  /**
   * Access the chip's raw registers.
   *
   * @return The chip's internal memory as a byte arry.
   */
  ...
  public synchronized byte read( int portId )
  {
    ...
    byte result = _registers[ portId ];

    switch ( portId )
    {
      // The set of registers handled by the default above.
      case PRA:
// Start 11:  Block in 11 removed
        // ...and mask out the write lines.  No 1 bits will be returned for
        // lines that are defined as output lines. See DDRA.

        // TODO check this out.  The code below doesn't work?  ...because of
        // using a single point of setting the PORT via register and hardware pins.
        //result &= (~_registers[ DDRA ]);

        // At last flip all bits, since the CIA is a low-active chip.
        // TODO the 'not' is wrong here!  It is not symmetric with the write.
...
// Start 11:  Block in 11 removed
        // ...and mask out the write lines.  No 1 bits will be returned for
        // lines that are defined as output lines. See DDRB.

        // TODO check this out.  The code below doesn't work? ...See comment above for details.
        //result &= (~_registers[ DDRB ]);

        // At last flip all bits, since the CIA is a low-active chip.
        result = (byte)~result;  // TODO the not is wrong here!
        break;
...
/* TODO 11
    // By default we update our register with the new value.  The switch below
    // only has to care for additional logic.
...
11 */
    switch ( portId )
    {
      ...
      case PRA:
        // TODO It needs to be clarified whether and how we have to take into
        // account the value of the DDRA register.
        // Write the value to the optional listener.
        if ( null != _ioPortA )
          _ioPortA.write( value );
        else // TODO Remove else in 11
          _registers[ PRA ] = value;
        break;

      // PRB -- Port register B.  State of pins PB0-7.  W:
      case PRB:
        // TODO It needs to be clarified whether and how we have to take into
        // account the value of the DDRB register.
        // Write the value to the optional listener.
        if ( null != _ioPortB )
          _ioPortB.write( value );
        else // TODO Remove else in 11
          _registers[ PRB ] = value;
        break;
